...
# Title: Hf Spectrum Simple Fosphor
...
class hf_spectrum_simple_fosphor(gr.top_block, Qt.QWidget):
    ...
    def __init__(self):
        ...
        for _label in self._ahfd_ant_labels: self._ahfd_ant_combo_box.addItem(_label)
        ...
        self._y_min_line_edit.returnPressed.connect(
            lambda: self.set_y_min(eng_notation.str_to_num(str(self._y_min_line_edit.text()))))
        ...
        self._y_max_line_edit.returnPressed.connect(
            lambda: self.set_y_max(eng_notation.str_to_num(str(self._y_max_line_edit.text()))))
...
def main(top_block_cls=hf_spectrum_simple_fosphor, options=None):
