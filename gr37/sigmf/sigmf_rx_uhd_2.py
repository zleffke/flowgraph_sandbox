...
class sigmf_rx_uhd_2(gr.top_block, Qt.QWidget):

    def __init__(self, addr='serial=314FC3C', note='wire format = complex int12; clock rate = 60M, samp_rate = 30M', path="/captures/20200223", signal_type='Lithium', usrp_type='B200'):
        ...
        self.settings = Qt.QSettings("GNU Radio", "sigmf_rx_uhd_2")
...
def argument_parser():
    ...
    parser.add_option(
        "", "--note", dest="note", type="string", default='wire format = complex int12; clock rate = 60M, samp_rate = 30M',
        help="Set note [default=%default]")
    parser.add_option(
        "", "--path", dest="path", type="string", default="/captures/20200223",
        help="Set path [default=%default]")
    parser.add_option(
        "", "--signal-type", dest="signal_type", type="string", default='Lithium',
...
def main(top_block_cls=sigmf_rx_uhd_2, options=None):
    if options is None:
