...
class rothr_playback_sigmf(gr.top_block, Qt.QWidget):
    ...
    def __init__(self):
        ...
        for c in range(4, 6):
            self.top_grid_layout.setColumnStretch(c, 1)
        self._coarse_freq_tool_bar = Qt.QToolBar(self)
        self._coarse_freq_tool_bar.addWidget(Qt.QLabel("coarse_freq"+": "))
        self._coarse_freq_line_edit = Qt.QLineEdit(str(self.coarse_freq))
        self._coarse_freq_tool_bar.addWidget(self._coarse_freq_line_edit)
        self._coarse_freq_line_edit.returnPressed.connect(
        	lambda: self.set_coarse_freq(eng_notation.str_to_num(str(self._coarse_freq_line_edit.text().toAscii()))))
        self.top_grid_layout.addWidget(self._coarse_freq_tool_bar, 5, 4, 1, 4)
        for r in range(5, 6):
            self.top_grid_layout.setRowStretch(r, 1)
        for c in range(4, 8):
            self.top_grid_layout.setColumnStretch(c, 1)
