...
# Title: Fsk Rx Uhd
# Generated: Tue Mar 27 23:59:36 2018
##################################################
...
class fsk_rx_uhd(gr.top_block, Qt.QWidget):
    ...
    def __init__(self, fsk_dev=10000, lpf_cutoff=12.5e3, lpf_trans=1e3, rx_freq=401.5e6, rx_offset=250e3):
...
def main(top_block_cls=fsk_rx_uhd, options=None):
    ...
    tb = top_block_cls(fsk_dev=options.fsk_dev, lpf_cutoff=options.lpf_cutoff, lpf_trans=options.lpf_trans, rx_freq=options.rx_freq, rx_offset=options.rx_offset)
    tb.start()
    tb.show()
