...
class spec_mon(gr.top_block, Qt.QWidget):

    def __init__(self):
        gr.top_block.__init__(self, "Spec Mon")
        Qt.QWidget.__init__(self)
        self.setWindowTitle("Spec Mon")
        ...
        self.settings = Qt.QSettings("GNU Radio", "spec_mon")
        ...
        for c in range(0, 2):
            self.top_grid_layout.setColumnStretch(c, 1)
        self._fine_range = Range(-1e3, 1e3, 1, 0, 200)
        self._fine_win = RangeWidget(self._fine_range, self.set_fine, "fine", "counter_slider", float)
        self.top_grid_layout.addWidget(self._fine_win, 5, 0, 1, 4)
        for r in range(5, 6):
            self.top_grid_layout.setRowStretch(r, 1)
        for c in range(0, 4):
            self.top_grid_layout.setColumnStretch(c, 1)
        self._coarse_range = Range(-10e3, 10e3, 1, 0, 200)
        self._coarse_win = RangeWidget(self._coarse_range, self.set_coarse, "coarse", "counter_slider", float)
        self.top_grid_layout.addWidget(self._coarse_win, 6, 0, 1, 4)
        for r in range(6, 7):
            self.top_grid_layout.setRowStretch(r, 1)
        for c in range(0, 4):
            self.top_grid_layout.setColumnStretch(c, 1)
        ...
        self.analog_sig_source_x_0 = analog.sig_source_c(samp_rate, analog.GR_COS_WAVE, -1 * (coarse+fine), 1, 0)
    ...
    def set_fine(self, fine):
        self.fine = fine
        self.analog_sig_source_x_0.set_frequency(-1 * (self.coarse+self.fine))
...
def main(top_block_cls=spec_mon, options=None):
